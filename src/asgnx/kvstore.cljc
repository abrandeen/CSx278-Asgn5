...
;;should I be using the action-assoc-in function???????????????????????????????????
...
(defn state-remove [m ks]
  (if (empty? (rest ks))
    (dissoc m (first ks))
   (let [result (state-remove (get m (first ks)) (rest ks))]
    (if (empty? result)
      (dissoc m (first ks))
      (assoc (dissoc m (first ks)) (first ks) result)))))
...
;; See the tests in asgnx.kvstore-test for a complete spec.
;;
(defn state-get [m ks] ;; Change the signature!
  (get-in m ks))
